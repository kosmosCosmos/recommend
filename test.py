...
def GetRecommendation(user, N):
    ...
    recommendListTopN = sorted(recommendDist.items(), key=lambda d: d[1], reverse=True)[0:N]
...
global trainSet
trainSet = loadDataSet('ml-100k/u1.base')
testSet = loadDataSet('ml-100k/u1.test')
global W
W = UserSimilarity(trainSet)
N = 3  # 此时 N＝ 10，代表对每个用户推荐10个物品
global K
#[3, 5, 10, 20, 40, 80, 160]
